...
5. **리서치**에서 회사, C레벨, 팀 리드 또는 팀원을 선택하고 신원 단서와 공개 URL을 입력합니다. 결과는 경력·기술 스택·프로젝트 근거를 추출하고, 선택한 지원자 문서와 공개 기술 근거의 교집합을 의사결정이 아닌 참고 의견으로 표시합니다. 사실·추론·조언을 구분해 각 주장에 출처를 연결하며 갱신 기록도 보존합니다.
6. 공고의 준비 화면에서 자기소개서, 이력서 피드백, 면접 질문, 기술 면접 질문, 토픽별 답안 또는 회사 질문을 선택합니다. 사용할 문서와 Provider를 고른 뒤 외부 전송 미리보기의 목적지·모델·정확한 입력 버전과 hash를 확인하고 동의해야 생성이 시작됩니다.

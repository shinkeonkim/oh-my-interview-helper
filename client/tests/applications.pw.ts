...
test("creates a posting and moves an application through the local pipeline", async ({ page }) => {
  ...
  await page.getByLabel("직무명").fill(post.title)
  await page.getByLabel("회사", { exact: true }).fill(post.companyName)
  await page.getByLabel("팀", { exact: true }).fill(post.teamName)
  await page.getByLabel("근무 위치").fill(post.metadata.location)
  await page.getByLabel("고용 형태").fill(post.metadata.employmentType)
  await page.getByLabel("공고 내용").fill("Role body")
  ...
  await versionCard.getByLabel("새 버전 출처").click()
  await page.getByRole("option", { name: "직접 입력" }).click()
  await versionCard.getByLabel("공고 내용").fill("Updated role responsibilities")
  ...
  await page.getByRole("combobox", { name: /^현재 단계:/ }).click()
  await page.getByRole("option", { name: "Applied" }).click()
  const applicationStage = page.getByRole("combobox", { name: /^현재 단계:/ })
  ...
  const interviewAt = page.getByLabel("면접 일시")
  ...
  await page.getByLabel("면접 종류").fill("Technical interview")
  await page.getByLabel("면접 장소 또는 링크").fill("https://meet.example.com/acme")
  await page.getByLabel("면접 준비 메모").fill("Review distributed systems examples")

...
const workflowGuidance = (workflow: PreparationWorkflowKind): string => {
  ...
  return [
    ...
    "modelAnswer must be a materially better version of likelyAnswer: retain only supported applicant facts while improving structure, specificity, reflection, relevance to company values, and culture-add. Never invent achievements, motives, actions, metrics, or experiences.",
    "When applicant documents do not support an answer, likelyAnswer must explicitly say which personal evidence is missing and modelAnswer must provide a fill-in structure rather than a fictional completed story.",
